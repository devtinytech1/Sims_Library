...
async function checkReavealedWild(client, matrix) {
  ...
  if (client.nextState === TRIGGER.SPIN && !client.node.context.freespins) {
    //to check will wild flip or not
    await wildFlippingProbInBaseGame(client, matrix)
  }
  //for freeSpin Wild check 
  else if (client.nextState === TRIGGER.SPIN && client.node.context.freespins && client.node.context.freespins.left) {
