...
const { getRandomItem, getRandomItems1, getRandomItemByArrayWeights } = require('../../../../../../src/game/runner/math/main_random_controller.cjs')
...
async function getMatrix(client, spinMod, rows) {
  ...
  if (settings.reelsSet[spinMod + '_Prob']) {
    ...
    const getReel = async function (ix) {
      const body = await getRandomItems1(client, sequences[ix], { count: rows, ix })
